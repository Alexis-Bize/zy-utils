...
export const assignErrors = (errors = []) => {
    ...
    errors.forEach(err => {
        ...
        if (length(err) === 0 || false === _.isPlainObject(err)) {
            continue;
        }

        err = toObject(Object.keys(err).map(key => {
            return { [key.toUpperCase()]: err[key].concat([key.toUpperCase()]) }
        }));

        Object.assign(appErrorKeys, err);
