 ...
 * @returns {Object}
 */
let appErrorKeys = toObject(
    Object.keys(BASE_ERROR_KEYS).map(key => ({
...
export const assignErrors = (errors = []) => {
    ...
    errors.forEach(err => {
        if (length(err) !== 0 && true === _.isPlainObject(err)) {
            Object.assign(
                appErrorKeys,
                toObject(
                    Object.keys(err).map(key => {
                        return {
                            [key.toUpperCase()]: err[key].concat([
                                key.toUpperCase()
                            ])
                        };
                    })
...
export default class Errors extends Error {
    ...
    constructor(error = [], additionalParameters = {}) {
        ...
        this.name = reason || error.reason || ERROR_NAME;
